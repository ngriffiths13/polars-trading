...
def standard_bars(
    ...
    """
    This function generates standard bars for a given DataFrame.
...
def tick_bars(
    ...
    """
    This function generates tick bars for a given DataFrame.
...
def volume_bars(
    ...
    size_col: str = "size",
    symbol_col: str = "symbol",
    bar_size: int | float | pl.Expr = 1_000_000,
):
    """
    This function generates volume bars for a given DataFrame.
    ...
    if isinstance(bar_size, int | float):
        df = df.with_columns(
            (pl.lit(bar_size).cast(pl.UInt32).alias("__PFIN_bar_size"))
        )
    elif isinstance(bar_size, pl.Expr):
        df = df.with_columns(bar_size.cast(pl.UInt32).alias("__PFIN_bar_size"))
    else:
        raise TypeError("bar_size must be an int, float, dict, or pl.Expr")
...
def dollar_bars(
    ...
    """
    This function generates dollar bars for a given DataFrame.
    ...
    if isinstance(bar_size, int | float):
        df = df.with_columns(
            (pl.lit(bar_size).cast(pl.Float64).alias("__PFIN_bar_size"))
        )
    elif isinstance(bar_size, pl.Expr):
        df = df.with_columns(bar_size.cast(pl.Float64).alias("__PFIN_bar_size"))
    else:
        raise TypeError("bar_size must be an int, float, or pl.Expr")
